...
function hasTransparency(context: CanvasContext, x0: number, y0: number, w: number, h: number): boolean {
...
function fixOpaqueSkin(context: CanvasContext, width: number, format1_8: boolean): void {
	...
	const clearArea = (x: number, y: number, w: number, h: number): void => context.clearRect(x * scale, y * scale, w * scale, h * scale);
...
function isAreaBlack(context: CanvasContext, x0: number, y0: number, w: number, h: number): boolean {
	const imgData = context.getImageData(x0, y0, w, h);
	for (let x = 0; x < w; x++) {
		for (let y = 0; y < h; y++) {
			const offset = (x + y * w) * 4;
...
function isAreaWhite(context: CanvasContext, x0: number, y0: number, w: number, h: number): boolean {
	const imgData = context.getImageData(x0, y0, w, h);
	for (let x = 0; x < w; x++) {
		for (let y = 0; y < h; y++) {
			const offset = (x + y * w) * 4;
...
export function inferModelType(canvas: TextureCanvas): ModelType {
	...
	const context = canvas.getContext('2d');
	const checkTransparency = (x: number, y: number, w: number, h: number): boolean => hasTransparency(context, x * scale, y * scale, w * scale, h * scale);
	const checkBlack = (x: number, y: number, w: number, h: number): boolean => isAreaBlack(context, x * scale, y * scale, w * scale, h * scale);
	const checkWhite = (x: number, y: number, w: number, h: number): boolean => isAreaWhite(context, x * scale, y * scale, w * scale, h * scale);
	const isSlim =
